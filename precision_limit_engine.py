...
class PrecisionLimitEngine:
    ...
    def _db_log(self, level: str, msg: str):
        """Write a concise message into DB logs (UI). Rate-limited."""
        try:
            # avoid spamming DB
            now = time.time()
            if now - getattr(self, "_last_db_log", 0.0) < 3.0 and level in ("DEBUG", "INFO"):
                return
            self._last_db_log = now
            self.db.add_log(level, msg)
        except Exception:
            # never break engine due to logging
            pass
    # ---------------------------------------------------------------------
    # 生命周期
    # ---------------------------------------------------------------------
    ...
    def start(self):
        ...
        self._thread.start()
        self._db_log('INFO', '限速引擎已启动（后台线程运行中）')
    ...
    def _run_loop(self):
        ...
        while self._running and not self._stop_event.is_set():
            ...
            try:
                ...
                self._db_log('ERROR', '限速引擎循环异常，请查看系统日志')
                min_tl = 9999.0
    ...
    def _process_all(self, now: float) -> float:
        ...
        # status hints for UI logs
        if not instances:
            if now - getattr(self, '_last_status_log', 0.0) > 30:
                self._last_status_log = now
                self._db_log('WARNING', '未检测到已连接的 qB 实例：限速不会生效（请在 qB 实例页检查连接）')
        ...
        for inst_id, by_limit in up_actions.items():
            for limit, hashes in by_limit.items():
                try:
                    ...
                    self._db_log('ERROR', f'上传限速下发失败：实例{inst_id} limit={limit}（查看系统日志）')
        ...
        for inst_id, by_limit in dl_actions.items():
            for limit, hashes in by_limit.items():
                try:
                    ...
                    self._db_log('ERROR', f'下载限速下发失败：实例{inst_id} limit={limit}（查看系统日志）')
        ...
        if controlled == 0 and instances and now - getattr(self, '_last_status_log', 0.0) > 30:
            self._last_status_log = now
            self._db_log('INFO', '未找到需要限速的活动种子：请确认规则已启用且种子处于 active 状态')
